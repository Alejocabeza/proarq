...
import { vatSchema } from "@app/schemas/vat.schema";
import { useCreateAdminExpensesMutation } from "@app/services/admin-expenses.service";
...
const Page = () => {
  ...
  const [createData, { isLoading }] = useCreateAdminExpensesMutation();
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit: SubmitHandler<VatInterface> = async (formValues) => {
    ...
    if (res.data.statusCode !== 201) {
      toast.error(res.data.message, {
        description: "Admin expenses Create Failed",
      });
      return;
    }
    toast.success(res.data.message, {
      description: "Admin expenses Create Successfully",
    });
    reset();
