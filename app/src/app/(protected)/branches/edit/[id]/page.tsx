...
import { branchUpdateSchema } from "@app/schemas/branch.schema";
import {
...
const Page = () => {
  ...
  const { id } = useParams();
  const { isLoading, data } = useFindOneBranchQuery(id);
  const [updateData, { isLoading: isUpdating }] = useUpdateBranchMutation();
  const {
    control,
    register,
    handleSubmit,
