...
import { useCreateClientMutation } from "@app/services/client.service";
...
const Page = () => {
  ...
  const [createData, { isLoading }] = useCreateClientMutation();
  const {
    control,
    register,
    handleSubmit,
  ...
  const onSubmit: SubmitHandler<ClientInterface> = async (formValues) => {
    ...
    if (res.data.statusCode !== 201) {
      toast.error(res.data.message, {
        description: "Client Create Failed",
      });
    }
    toast.success(res.data.message, {
      description: "Client Create Successfully",
    });
    reset();
    router.push("/clients");
  };
