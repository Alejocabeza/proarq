...
import { clientUpdateSchema } from "@app/schemas/client.schema";
import {
...
const Page = () => {
  ...
  const { id } = useParams();
  const { isLoading, data } = useFindOneClientQuery(id);
  const [updateData, { isLoading: isUpdating }] = useUpdateClientMutation();
  const {
    control,
    register,
    handleSubmit,
  ...
  const onSubmit: SubmitHandler<ClientInterface> = async (formValues) => {
    ...
    toast.success(res.data.message, {
      description: "Client Update Successfully",
    });
  };
