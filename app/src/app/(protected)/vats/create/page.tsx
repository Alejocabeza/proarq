...
import { useCreateVatMutation } from "@app/services/vat.service";
...
const Page = () => {
  ...
  const [createData, { isLoading }] = useCreateVatMutation();
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit: SubmitHandler<VatInterface> = async (formValues) => {
    ...
    if (res.data.statusCode !== 201) {
      toast.error(res.data.message, {
        description: "Vat Create Failed",
      });
      return;
    }
    toast.success(res.data.message, {
      description: "Vat Create Successfully",
    });
    reset();
    router.push("/vats");
  };
