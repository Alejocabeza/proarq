...
import { vatUpdateSchema } from "@app/schemas/vat.schema";
import {
  useDeleteVatMutation,
  useFindOneVatQuery,
  useUpdateVatMutation,
} from "@app/services/vat.service";
...
const Page = () => {
  ...
  const { id } = useParams();
  const { isLoading, data } = useFindOneVatQuery(id);
  const [updateData, { isLoading: isUpdating }] = useUpdateVatMutation();
  const {
    register,
    handleSubmit,
    formState: { errors },
    setValue,
  } = useForm<VatInterface>({
    resolver: zodResolver(vatUpdateSchema),
    values: {
      ...data,
    },
  });
  const [deleteData] = useDeleteVatMutation();
  ...
  const onSubmit: SubmitHandler<VatInterface> = async (formValues) => {
    ...
    if (res.data.statusCode !== 200) {
      toast.error(res.data.message, {
        description: "Vat Update Failed",
      });
      return;
    }
    toast.success(res.data.message, {
      description: "Vat Update Successfully",
    });
  };
